...
def explode_matches(spark: SparkSession, PineconeLookup_1: DataFrame) -> DataFrame:
    ...
    selectCols = [col("pinecone_matches") if "pinecone_matches" in flt_col else col("pinecone_matches"),                   col("input") if "input" in flt_col else col("text").alias("input"),                   col("channel") if "channel" in flt_col else col("channel"),                   col("ts") if "ts" in flt_col else col("ts")]
